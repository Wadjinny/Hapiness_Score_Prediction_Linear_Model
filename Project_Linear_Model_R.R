...
y <- scale(df$Score) #define scaled vector of Score
x <- scale(data.matrix(df[, c('GDP_per_capita', 'Social_support', 'Healthy_life_expectancy',
                        'Freedom_to_make_life_choices', 'Perceptions_of_corruption')])) #define scaled matrix of predictive variables
...
RMSE_Ridge <- (mean((y_predicted_Ridge - y)**2))**0.5
RMSE_Ridge #RMSE_Ridge = 0.4699795

SST_1 <- sum((y - mean(y))**2) #compute SST_1
SSE_1 <- sum((y_predicted_Ridge - y)**2) #compute SSE_1

rsq_1 <- 1 - SSE_1/SST_1 #computing R-squared
rsq_1 #rsq_1 = 0.7776942 is slightly better than the r-squared for the new_model (rsq = 0.7703) 
...
RMSE_LASSO <- (mean((y_predicted_LASSO - y)**2))**0.5
RMSE_LASSO #RMSE_LASSO = 0.4709764

SST_2 <- sum((y - mean(y))**2) #compute SST_2
SSE_2 <- sum((y_predicted_LASSO - y)**2) #compute SSE_2

rsq_2 <- 1 - SSE_2/SST_2 #computing R-squared
rsq_2 #rsq_2 = 0.7767501 is slightly better than the r-squared for the new_model (rsq = 0.7703)
